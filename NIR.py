...
def crop_edges(image):
    # Assuming cropping edges by 10 pixels on each side as an example
    cropped_image = image[10:-10, 10:-10]
    return cropped_image

def convert_rgb_to_nir_frames(rgb_frames):
    cir_frames = [convert_to_infrared(frame) for frame in rgb_frames]
    return cir_frames
